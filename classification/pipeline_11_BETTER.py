...
def classify_and_assign_news(limit=5, csv_filename="news_classification.csv"):
    ...
    with open(csv_filename, mode='w', newline='', encoding='utf-8') as csv_file:
        ...
        for article in news_articles:
            ...
            prompt = (
                "You are an assistant tasked with classifying news articles. "
                "Determine if the news article is related to specific American cities or if it is global. "
                "If the news is related to specific American cities, list the city names separated by commas. "
                "If the news is not related to any specific American city, simply write 'Global'.\n"
                f"Title: {title}\nContent: {content}\n"
            )
            ...
            try:
                ...
                city_names = [normalize_city_name(city) for city in output.split(',') if city.strip()]

                if city_names:
                    if "global" in city_names:  # If 'Global' is returned
                        print(f"News id {news_id} marked as global.")
                        # Check if the global news entry already exists in CityNews
                        ...
                        for city_name in city_names:
                            ...
                            # Use LIKE to find the city in the database with the first 4 characters of the city name
                            like_pattern = f"%{city_name[:4]}%"
                            ...
                            if city:
                                ...
                                # Check if the city-news relationship already exists
